...
function Tile({ title, routeName, onPress, theme }) {
  ...
  return (
    <Pressable onPress={onPress} onPressIn={handlePressIn} onPressOut={handlePressOut} style={{ margin: 6 }} accessibilityRole="button">
      <Animated.View style={[styles.tile, { backgroundColor: theme.colors.buttonPrimaryBg, transform: [{ scale }] }]}>
        <Text style={[styles.tileText, { color: theme.colors.buttonPrimaryText }]}>{title}</Text>
      </Animated.View>
    </Pressable>
  );
}
...
export default function TilesScreen() {
  ...
  return (
    ...
    >
      <Text className="px-2 mt-4" style={[styles.title, { color: theme.colors.headingText }]}>
        Quick Tiles
      </Text>

      <View style={styles.grid}>
        {tiles.map((t) => (
          <Tile key={t.id} title={t.title} routeName={t.route} onPress={() => handleTilePress(t)} theme={theme} />
        ))}
      </View>
    </View>
  );
